...
void exec_st_ter(int *param1, vm_t *vm, champion_t *champion, int *type)
{
    *param1 = champion->r[(int) vm->arena[my_modulo(champion->pc + 2,
    MEM_SIZE)]];
...
{
    if (type == 1) {
        if (!is_valid_register((int) vm->arena[my_modulo(champion->pc + 3,
        MEM_SIZE)], champion))
            return 1;
        champion->r[(int) vm->arena[my_modulo(champion->pc + 3,
        MEM_SIZE)]] = param1;
    }
    return 0;
}
...
void exec_st(vm_t *vm, champion_t *champion)
{
    ...
    MEM_SIZE)], champion))
        return;
    exec_st_ter(&param1, vm, champion, &type);
