...
int get_sti_param3(vm_t *vm, champion_t *champion, int *act_pc, int *param3)
{
    ...
    set_bit_int_at(&type, 0, get_bit_at(vm->arena[my_modulo(champion->pc + 1, MEM_SIZE)], 4));
    set_bit_int_at(&type, 1, get_bit_at(vm->arena[my_modulo(champion->pc + 1, MEM_SIZE)], 5));
    set_bit_int_at(&type, 0, get_bit_at(vm->arena[my_modulo(champion->pc + 1, MEM_SIZE)], 2));
    set_bit_int_at(&type, 1, get_bit_at(vm->arena[my_modulo(champion->pc + 1, MEM_SIZE)], 3));
...
void exec_sti(vm_t *vm, champion_t *champion)
{
    ...
    if (get_sti_param3(vm, champion, &act_pc, &param3))
        return;
    write_four_bytes(vm->arena, my_modulo((champion->pc + (param2 + param3) % IDX_MOD), MEM_SIZE),
