...
int exec_lld_bis(int type, vm_t *vm, champion_t *champion, int param1)
{
    if (type == 2) {
        if (!is_valid_register((int) vm->arena[my_modulo(champion->pc + 6, MEM_SIZE)], champion))
            return 1;
        champion->r[(int) vm->arena[my_modulo(champion->pc + 6, MEM_SIZE)]] = param1;
        champion->carry = (param1 == 0) ? 1 : 0;
    }
    if (type == 3) {
        if (!is_valid_register((int) vm->arena[my_modulo(champion->pc + 4, MEM_SIZE)], champion))
            return 1;
        champion->r[(int) vm->arena[my_modulo(champion->pc + 4, MEM_SIZE)]] =
        vm->arena[my_modulo(champion->pc + param1, MEM_SIZE)];
...
void exec_lld(vm_t *vm, champion_t *champion)
{
    ...
    param1 = get_value_of_param(vm, type, my_modulo(champion->pc + 2, MEM_SIZE));
    if (exec_lld_bis(type, vm, champion, param1))
        return;
