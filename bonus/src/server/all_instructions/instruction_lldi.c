...
int get_value_indirect_lldi(vm_t *vm, champion_t *champion, int copy)
{
    int param = 0;
    int k = 1;
    ...
    for (int i = 0; i != 2; i++) {
        for (int j = 0; j != 8; j++)
            ...
            get_bit_at(vm->arena[champion->pc + copy + i], j));
        k--;
    }
    return param;
}

int get_lldi_param1(vm_t *vm, champion_t *champion, int *act_pc, int *param1)
{
    int type = 0;
    int copy = 0;
    ...
    *param1 = analyze_type(type, act_pc, champion, vm);
    ...
    copy = *param1;
    if (type == 3)
        *param1 = get_value_indirect_lldi(vm, champion, copy);
    return 0;
}

int get_lldi_param2(vm_t *vm, champion_t *champion, int *act_pc, int *param2)
{
    int type = 0;
...
void exec_lldi(vm_t *vm, champion_t *champion)
{
    ...
    if (!is_valid_instruction(vm, champion))
        return;
    if (get_lldi_param1(vm, champion, &act_pc, &param1))
        return;
    if (get_lldi_param2(vm, champion, &act_pc, &param2))
        return;
    if (!is_valid_register((int) vm->arena[act_pc], champion))
        return;
    champion->r[(int) vm->arena[act_pc]] =
    vm->arena[my_modulo(champion->pc + (param1 + param2), MEM_SIZE)];
