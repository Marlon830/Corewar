...
int get_or_param1(vm_t *vm, champion_t *champion, int *act_pc, int *param1)
{
    int type = 0;
...
int get_or_param2(vm_t *vm, champion_t *champion, int *act_pc, int *param2)
{
    int type = 0;
...
void exec_or(vm_t *vm, champion_t *champion)
{
    ...
    if (!is_valid_instruction(vm, champion))
        return;
    if (get_or_param1(vm, champion, &act_pc, &param1))
        return;
    if (get_or_param2(vm, champion, &act_pc, &param2))
        return;
